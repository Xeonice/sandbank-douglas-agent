...
### Added
- `connectTerminal()` in `@douglas-agent/sandbank-core` — wraps ttyd WebSocket binary protocol into `TerminalSession` interface with `write`/`onData`/`resize`/`close`
...
- `@douglas-agent/sandbank-relay` package resolution failure in tests (missing `dist/` directory)
...
#### Core (`@douglas-agent/sandbank-core`)
...
#### Adapters
- `@douglas-agent/sandbank-daytona` — Daytona cloud VM adapter (volumes, port.expose, terminal)
- `@douglas-agent/sandbank-flyio` — Fly.io Machines adapter, zero external dependencies (volumes, port.expose, terminal)
- `@douglas-agent/sandbank-cloudflare` — Cloudflare Workers adapter (exec.stream, snapshot, volumes, port.expose, terminal)

#### Communication
- `@douglas-agent/sandbank-relay` — WebSocket + HTTP relay server with JSON-RPC 2.0 protocol
- `@douglas-agent/sandbank-agent` — Lightweight in-sandbox client with `connect()`, messaging, and shared context
