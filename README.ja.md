...
│  アプリケーション / AI エージェント                      │
├──────────────────────────────────────────────────────┤
│  @douglas-agent/sandbank-core         統一プロバイダーインターフェース  │
│  @douglas-agent/sandbank-skills       スキルレジストリ・インジェクション │
│  @douglas-agent/sandbank-agent        サンドボックス内エージェント      │
│  @douglas-agent/sandbank-relay        マルチエージェント通信            │
├──────────────────────────────────────────────────────┤
...
| パッケージ | 説明 |
|-----------|------|
| [`@douglas-agent/sandbank-core`](./packages/core) | プロバイダー抽象化、ケイパビリティシステム、エラー型 |
| [`@douglas-agent/sandbank-skills`](./packages/skills) | スキルレジストリ、ローカルファイルシステムローダー |
| [`@douglas-agent/sandbank-daytona`](./packages/daytona) | Daytona クラウドサンドボックスアダプター |
| [`@douglas-agent/sandbank-flyio`](./packages/flyio) | Fly.io Machines アダプター |
| [`@douglas-agent/sandbank-cloudflare`](./packages/cloudflare) | Cloudflare Workers アダプター |
| [`@douglas-agent/sandbank-boxlite`](./packages/boxlite) | BoxLite セルフホスト Docker アダプター |
| [`@douglas-agent/sandbank-relay`](./packages/relay) | マルチエージェント通信用 WebSocket リレー |
| [`@douglas-agent/sandbank-agent`](./packages/agent) | サンドボックス内エージェント軽量クライアント |
...
サンドボックス内では、エージェントは `@douglas-agent/sandbank-agent` を使用します：
...
```bash
# インストール
pnpm add @douglas-agent/sandbank-core @douglas-agent/sandbank-daytona  # または @douglas-agent/sandbank-flyio、@douglas-agent/sandbank-cloudflare
