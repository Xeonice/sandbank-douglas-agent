...
import type { SandboxProvider, Sandbox } from '@douglas-agent/sandbank-core'
...
if (process.env.DAYTONA_API_KEY) {
  ...
  const { DaytonaAdapter } = await import('@douglas-agent/sandbank-daytona')
...
if (process.env.FLY_API_TOKEN && process.env.FLY_APP_NAME) {
  const { FlyioAdapter } = await import('@douglas-agent/sandbank-flyio')
...
if (process.env.BOXLITE_API_URL && (process.env.BOXLITE_API_TOKEN || (process.env.BOXLITE_CLIENT_ID && process.env.BOXLITE_CLIENT_SECRET))) {
  const { BoxLiteAdapter } = await import('@douglas-agent/sandbank-boxlite')
...
for (const entry of providers) {
  describe(`conformance: ${entry.name}`, () => {
    ...
    it('stream output contains expected content', async () => {
      ...
      const streamable = sandbox as import('@douglas-agent/sandbank-core').StreamableSandbox
    ...
    it('startTerminal returns { url, port }', async () => {
      ...
      const terminal = sandbox as import('@douglas-agent/sandbank-core').TerminalSandbox
    ...
    it('exposePort returns a URL string', async () => {
      ...
      const exposable = sandbox as import('@douglas-agent/sandbank-core').PortExposeSandbox
