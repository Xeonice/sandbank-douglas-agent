...
│  你的应用 / AI Agent                                  │
├──────────────────────────────────────────────────────┤
│  @douglas-agent/sandbank-core         统一 Provider 接口            │
│  @douglas-agent/sandbank-skills       Skill 注册表与注入            │
│  @douglas-agent/sandbank-agent        沙箱内 Agent 客户端           │
│  @douglas-agent/sandbank-relay        多 Agent 通信中枢             │
├──────────────────────────────────────────────────────┤
...
| 包名 | 说明 |
|------|------|
| [`@douglas-agent/sandbank-core`](./packages/core) | Provider 抽象、能力系统、错误类型 |
| [`@douglas-agent/sandbank-skills`](./packages/skills) | Skill 注册表、本地文件系统加载器 |
| [`@douglas-agent/sandbank-daytona`](./packages/daytona) | Daytona 云沙箱适配器 |
| [`@douglas-agent/sandbank-flyio`](./packages/flyio) | Fly.io Machines 适配器 |
| [`@douglas-agent/sandbank-cloudflare`](./packages/cloudflare) | Cloudflare Workers 适配器 |
| [`@douglas-agent/sandbank-boxlite`](./packages/boxlite) | BoxLite 自托管 Docker 适配器 |
| [`@douglas-agent/sandbank-relay`](./packages/relay) | WebSocket 中继，用于多 Agent 通信 |
| [`@douglas-agent/sandbank-agent`](./packages/agent) | 沙箱内 Agent 轻量客户端 |
...
在沙箱内部，Agent 使用 `@douglas-agent/sandbank-agent`：
...
```bash
# 安装
pnpm add @douglas-agent/sandbank-core @douglas-agent/sandbank-daytona  # 或 @douglas-agent/sandbank-flyio、@douglas-agent/sandbank-cloudflare
